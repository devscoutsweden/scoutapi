...
=== Creating API Users Manually

You can create users without associated identities, e.g. accounts for robots and other services, by executing these SQL
commands (this example creates a moderator called "Auto-updater" with a 50-character API key):

    INSERT INTO users (display_name, created_at, updated_at, role) VALUES (
        'Auto-updater',
        LOCALTIMESTAMP,
        LOCALTIMESTAMP,
        10);

    INSERT INTO user_api_keys (key, created_at, updated_at, user_id) VALUES (
        ARRAY_TO_STRING(ARRAY(SELECT SUBSTR('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', TRUNC(RANDOM() * 62)::integer + 1, 1) FROM   GENERATE_SERIES(1, 50)), ''),
        LOCALTIMESTAMP,
        LOCALTIMESTAMP,
        (SELECT ID FROM users WHERE display_name = 'Auto-updater')
        );

Note that the SQL syntax is specific to PostgreSQL.
...
The only way of creating administrators is to either create the user from scratch, as shown above, or to update the
role attribute for an existing user. Both methods require direct access to the database, i.e. a way of sending SQL
commands to the database.

Step 1) Find the user:
select * from users;

Step 2) Change the user's role (in this case the role is set for user 2):
update users set role = 20 where id = 2;
...
The only way of creating an administrator when there are no pre-existing adminsitrators is to update the database manually.

1. Create the user by connecting to the API using a Google id token. The user will be automatically created.
2. Connect to the database using the database tool of your choice.
3. Find the user's row in the users table.
4. Set the role value to 20, or whatever level the API implementation maps to the administrator role.
