...
The API supports searching for multiple words, all of which must be present in each matched activity. Each word is
separated by a space. Additionally, the API also supports excluding activities containing words prefixed by a minus.

Some examples:

    The text or name parameter  Interpretation
    --------------------------  --------------
    cat                         Activities containing "cat", "cats", "catepillar", "scattered" and so on.
    cat dog                     Activities which contains "cat" at least once and "dog" at least once.
    cat -dog -catepillar        Activitiies with "cat" and "dog" but not "catepillar".
...
Attaching (uploading) a photo to an activity requires a few steps:

=== 1. Upload the image by POSTing to api/v1/media_files
...
=== 2. Get activity's current list of associated media items by GETting api/v1/activities/999?attrs%5B%5D=media_files
...
=== 3. Patch the activity by PATCHing api/v1/activities/999

Sample request:

    PATCH http://127.0.0.1:3000/api/v1/activities/999 HTTP/1.1
    Authorization: Token token="664a2072e2"
    {
       "media_files": [111, 112, 113]
    }
